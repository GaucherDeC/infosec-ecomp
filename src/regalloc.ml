...
let add_interf (rig : (reg, reg Set.t) Hashtbl.t) (x: reg) (y: reg) : unit =
  let lx = match Hashtbl.find_option rig x with
    |None -> Set.singleton y
    |Some s -> Set.add y s
  in
  let ly = match Hashtbl.find_option rig y with
    |None -> Set.singleton x
    |Some s -> Set.add x s
  in
  Hashtbl.replace rig x lx;
  Hashtbl.replace rig y ly
  ...
  let rec f i a =
    let rec add_i r n1 =
      if Set.is_empty n1 then ()
        ...
        add_i r n1
    in
    if Set.is_empty a then ()
    else
      let r = Set.choose a in
      let n = Set.remove r a in
      add_i r n;
      f i n
  in
  Hashtbl.iter f live
...
let pick_node_with_fewer_than_n_neighbors (rig : (reg, reg Set.t) Hashtbl.t) (n: int) : reg option =
  let a = Hashtbl.filter( fun x -> Set.cardinal x < n) rig
  in
  if Hashtbl.is_empty a then None else Some (fst (List.first (Hashtbl.to_list a)))
...
let allocate (allocation: (reg, loc) Hashtbl.t) (rig: (reg, reg Set.t) Hashtbl.t)
  ...
  : int =
  let color r =
    let a = Hashtbl.find rig r in
    let aux k v = match v with
    | Stk s -> None
    | Reg c -> if Set.mem k a then Some c else None
    in
