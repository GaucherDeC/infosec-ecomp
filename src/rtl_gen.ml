...
let rec rtl_instrs_of_cfg_expr (next_reg, var2reg) (e: expr) =
  match e with
  |Evar s -> let r, next_reg, var2reg = find_var (next_reg, var2reg) s in (r,[],next_reg, var2reg)
  ...
  |Ebinop (op,e1,e2) -> 
    ...
    (next_reg,l1@l2@[Rbinop (op,next_reg, r1, r2)],next_reg+1, var2reg)
  |Eunop (op,e)-> let r, l, next_reg, var2reg = rtl_instrs_of_cfg_expr (next_reg, var2reg) e in
  (next_reg,l@ [Runop(op, next_reg, r)], next_reg+1, var2reg)
...
let rtl_instrs_of_cfg_node ((next_reg:int), (var2reg: (string*int) list)) (c: cfg_node) =
  match c with
  | Cassign (v,e,s)-> 
    let r1, next_reg, var2reg = find_var (next_reg, var2reg) v in
    let r2, l, next_reg, var2reg = rtl_instrs_of_cfg_expr (next_reg, var2reg) e in
    (l@ [Rmov (r1,r2);Rjmp s], next_reg, var2reg)
  | Creturn e -> let r, l , next_reg, var2reg = rtl_instrs_of_cfg_expr (next_reg,var2reg) e in
    (l@[Rret r], next_reg, var2reg)
  | Cprint (e, next) -> let r, l, next_reg, var2reg = rtl_instrs_of_cfg_expr (next_reg, var2reg) e in
    (l @ [Rprint r] @ [Rjmp next], next_reg, var2reg)
  | Ccmp (e,i1,i2) -> let cmp, e1, e2 = rtl_cmp_of_cfg_expr e in
    let r1, l1, next_reg,var2reg = rtl_instrs_of_cfg_expr (next_reg, var2reg) e1 in
    let r2, l2, next_reg,var2reg = rtl_instrs_of_cfg_expr (next_reg, var2reg) e2 in
    (l1@l2@[Rbranch (cmp, r1, r2, i1); Rjmp i2], next_reg, var2reg)
